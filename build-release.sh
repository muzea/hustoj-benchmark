...
for os in ${OSES[@]}; do
	...
	tar -zcf timetest-${os}-amd64-$VERSION.tar.gz timetest_${os}_amd64${suffix}
done
...
for os in ${OSES[@]}; do
	...
	tar -zcf timetest-${os}-386-$VERSION.tar.gz timetest_${os}_386${suffix}
done
...
tar -zcf timetest-linux-arm$v-$VERSION.tar.gz timetest_linux_arm$v
done
...
tar -zcf timetest-linux-arm64-$VERSION.tar.gz timetest_linux_arm64
...
tar -zcf timetest-linux-mipsle-$VERSION.tar.gz timetest_linux_mipsle
tar -zcf timetest-linux-mips-$VERSION.tar.gz timetest_linux_mips
